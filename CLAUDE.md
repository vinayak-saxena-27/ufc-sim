...
- Apex FC has a documented tendency toward talent over-concentration relative to the other two top-tier orgs over very long simulation runs. As of 2026-07-13, The League/Eastern GP each have an enforced per-org minimum (`career/replenishment.py::TIER4_ORG_FLOORS`) so they can no longer collapse toward zero regardless of poaching — but Apex still tends to sit well above its own floor while the other two hover nearer theirs, so the *relative* concentration bias isn't fully eliminated, just floored.
